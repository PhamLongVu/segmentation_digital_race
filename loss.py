...
def SegmentationLoss(preds, targets):
    # Chúng ta sẽ sử dụng Cross-Entropy Loss cho bài toán segmentation với N lớp
    ce_loss = F.cross_entropy(preds, targets, reduction='mean')
    
    # Kết hợp với các thành phần khác như Dice Loss và Lovasz Loss (nếu cần)
    dice = dice_loss(preds, targets)
    lovasz = lovasz_hinge_flat(preds, targets)
    
    total_loss = ce_loss + dice * config.DICE_COEF + lovasz * config.LOVASZ_COEF
    return total_loss
